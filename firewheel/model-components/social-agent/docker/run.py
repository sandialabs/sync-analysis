...
with open(os.path.join(DIR, 'frankenstein.txt')) as fd:
    WORDS = ''.join(x.lower() for x in fd.read()
                    if x.isascii() and x.isalpha() or x.isspace()).split()

NUM_WORDS = 8

def call(query):
    result = requests.post(f'http://{env["JOURNAL"]}/.interface', '(*local* "{}" {})'.format(
        env['SECRET'],
        query,
    )).text
...
def run(peers):
    ...
    for peer in peers[env['JOURNAL']]:
        while r := call('(ledger-peer! {} (lambda (msg) (sync-remote "{}" msg)))'.format(
            peer.rsplit('.', 1)[0],
    ...
    while True:
        ...
        until += timedelta(seconds=exponential(2**(sum([
            int(env["PERIODICITY"]),
            -int(env["ACTIVITY"]),
        ]))))

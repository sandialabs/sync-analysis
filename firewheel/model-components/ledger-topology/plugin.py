...
from synchronic_web.journal import Journal
...
class Plugin(AbstractPlugin):
    ...
    def split_seq(self, agents, num_agent_routers):
        ...
        for i in range(num_agent_routers):
            newseq.append(
                agents[int(round(i * splitsize)):int(round((i + 1) *
                                                           splitsize))])
        return newseq
    ...
    def run(self, number, periodicity, server="True"):
        ...
        for i in range(num_agents):
            ...
            agent.decorate(Journal, init_args=[f"pass-{i}", periodicity_val, server_val])
            agents.append(agent)

        max_bgp_routers = 219
        if num_routers > max_bgp_routers:  #Number of non-reserved and non-private, etc. /8 ranges
        ...
        leaves = [
        ]  #will be a list of dicts where the index is router number. Dicts contain leaf, bgp_net, switch, bgp_ips
        ...
        for i in range(num_routers):
            ...
            bgp_router = self._make_router_pair('leaf-%d.net' % (i,), control_nets, as_nums, \
                hosts_to_add)
            ...
            for edge in topology.edges:
                ...
                if not switch_exists:
                    ...
                    switch.decorate(Switch,
                                    init_args=['root-leaf%d.switch' % (i, )])
                    switch_exists = True
                    ip = next(bgp_ips)
                    bgp_to_switch_ip = ip
                ...
                ip = next(bgp_ips)
                neighbor_list[bgp_router.name][
                    connect_obj['bgp_router'].name] = str(ip)
                neighbor_list[connect_obj['bgp_router'].name][
                    bgp_router.name] = str(bgp_to_switch_ip)
                connect_obj['bgp_router'].connect(switch, ip, bgp_netmask)
                self.host_ip_map.setdefault(connect_obj['bgp_router'].name,
                                            []).append(ip)
        ...
        for vertex in self.g.get_vertices():
            if vertex.is_decorated_by(Journal):
                ...
                assert len(vertex.interfaces.interfaces
                           ) != 0, "Vertex {} had no interfaces".format(
                               vertex.name)
    ...
    def _make_router_pair(self, name, control_nets, as_nums,
                          agents_to_connect):
        ...
        switch_o_b.decorate(Switch,
                            init_args=['switch-ospf-bgp.%s' % (name, )])
